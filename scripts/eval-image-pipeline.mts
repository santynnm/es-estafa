...
async function extractImage(image_base64: string, mime_type: string) {
...
async function main() {
  ...
  {
    const { status, body } = await extractImage(b64("scam-message.png"), "image/png");
  ...
  {
    const { status, body } = await extractImage(b64("scam-message.webp"), "image/webp");
    const rawText = (body as { raw_text?: string } | null)?.raw_text ?? "";
  ...
  // 3. MIME inválido.
  {
    const { status, body } = await extractImage(b64("scam-message.png"), "image/gif");
  ...
  // 4. Base64 inválido.
  {
    const { status, body } = await extractImage("esto-no-es-base64-!!!valido", "image/png");
    const ok = status === 400 && typeof (body as { error?: unknown } | null)?.error === "string";
  ...
  // validación (400 JSON) en vez del 413 genérico de la plataforma.
  {
    ...
    const { status, body } = await extractImage(big, "image/png");
    const ok = status === 400 && typeof (body as { error?: unknown } | null)?.error === "string";
  ...
  {
    const { status, body } = await extractImage(b64("no-text.png"), "image/png");
  ...
  {
    const extracted = await extractImage(b64("scam-message.png"), "image/png");
